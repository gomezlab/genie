...
for train_index, test_index in skf.split(X, y, groups):
    ...
    if data_type != 'comb':
        ...
        num_0 = len(X_test[X_test['clin_stage_dx'] == 0])
        X_train = pd.concat([X_train, X_test[X_test['clin_stage_dx'] == 0]])
        X_test = X_test[X_test['clin_stage_dx'] != 0]
        y_train = pd.concat([y_train, y_test[X_test['clin_stage_dx'] == 0]])
        y_test = y_test[X_test['clin_stage_dx'] != 0]
        #get a random sample of size num_0 from X_train where clin_stage_dx is 1 and move to X_test
        X_0_samp = X_train[X_train['clin_stage_dx'] == 1].sample(n=num_0, random_state=1)
        X_train = X_train.drop(X_0_samp.index)
        X_test = pd.concat([X_test, X_0_samp])
        y_0_samp = y_train[X_train['clin_stage_dx'] == 1].sample(n=num_0, random_state=1)
        y_train = y_train.drop(y_0_samp.index)
        y_test = pd.concat([y_test, y_0_samp])        
